...
for i = 1 : sim_time
    ...
    full_msg(satellites_t{sat_n,1}+1,fs*(i-1)+1:fs*i) = msg_mac;   %Includes de samples on the main matrix
    end
...
for sat_n = 1:nsat
    disp("Writing data of satellite " + (sat_n-1))    %Indicator for runtime
    write_file(full_msg(sat_n,1:end),sat_n-1);        %Function to write I-Q samples of an specific satellite
end
